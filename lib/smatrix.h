...
template <typename Float=double, typename Index=std::size_t>
  ...
  class SparseMatrix {
    ...
    std::istream& read(std::istream &data)
    {
        Float val;
        int m,n;
        data >> m >> n;
    ...
    std::ostream& write(std::ostream& data)
    {
      data << std::endl;
      for (unsigned long k=1; k<=this->rows(); k++)
        {
          data << k << " : ";
          double* n = this->begin(k);
          double* e = this->end  (k);
          for(std::size_t* i=this->ibegin(k) ; n!=e; n++, i++)
            {
              data << *n << " [" << *i << "]  ";
            }
          data << std::endl;
